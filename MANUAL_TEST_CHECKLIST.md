...
- [ ] View/List 모드에서 `:` 입력 후 `:e`, `:q`가 의도한 패널 전환으로 동작한다.
- [ ] 저장 실패를 유도했을 때(예: URI 오타) 편집 내용이 유지되고 종료/이동되지 않는다.
- [ ] `Cmd/Ctrl + S`, 저장 버튼, `:w`가 동일 결과를 낸다.

## 4) 레이아웃/스크롤/푸터
- [ ] View 모드에서 `좌측 목록 + 우측 뷰어` 2단 레이아웃이 유지된다.
- [ ] Write 모드 진입 시 좌측 목록이 숨겨지고 `에디터 + Preview` 좌우 2단이 유지된다.
- [ ] 좌측 목록 스크롤과 우측 패널 스크롤이 서로 간섭하지 않는다.
- [ ] 하단 커맨드 푸터가 List/View/Write 모든 모드에서 지속적으로 보인다.
...
- [ ] orphan 이미지는 배치 정리(1일 1회, 24시간 보호) 정책으로 정리되고 실패 시 앱 동작은 유지된다.

## 7) 반응형/모바일 점검
- [ ] 좁은 해상도(예: 980px 이하)에서 레이아웃이 1열로 전환되어 입력/조회가 가능하다.
- [ ] 좁은 해상도에서도 커맨드 푸터가 가려지지 않고 명령 입력이 가능하다.
- [ ] 모바일 브라우저에서 키보드 오픈 시 입력 필드/푸터 접근성이 유지된다.
